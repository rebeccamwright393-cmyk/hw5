...
export default function Chat({ user, onLogout }) {
  ...
  const loadChannelJson = async (file) => {
    try {
      const text = await fileToText(file);
      const parsed = JSON.parse(text);
      const videos = getVideosFromChannelData(parsed);
      setChannelData(parsed);
      setChannelDataError(null);
      localStorage.setItem(CHANNEL_STORAGE_KEY, text);
    } catch (err) {
      setChannelDataError(err.message || 'Invalid JSON');
      setChannelData(null);
  ...
  const handleDrop = async (e) => {
    ...
    if (jsonFiles.length > 0) {
      await loadChannelJson(jsonFiles[0]);
    }
  ...
  const handleSend = async () => {
    ...
    if ((!text && !images.length && !csvContext && !channelData) || streaming || !activeSessionId) return;
...
`
    ...
    // channelData prefix — include YouTube channel JSON in chat context when loaded
    const channelDataPrefix = channelData
      ? `[YouTube channel JSON loaded — ${getVideosFromChannelData(channelData).length} videos]\n\n\`\`\`json\n${JSON.stringify(channelData).slice(0, 150000)}\n\`\`\`\n\n---\n\n`
      : '';
    ...
    try {
      if (useChannelTools) {
        ...
        for await (const chunk of streamChat(history, promptForGemini, imageParts, useCodeExecution, user?.firstName)) {
    ...
    <div className="chat-layout">
      ...
      <aside className="chat-sidebar">
        ...
        <div className="sidebar-footer">
          <span className="sidebar-username">{user?.firstName ? `${user.firstName} (${user.username})` : username}</span>
          ...
          {messages.map((m) => (
            <div key={m.id} className={`chat-msg ${m.role}`}>
              <div className="chat-msg-meta">
                <span className="chat-msg-role">{m.role === 'user' ? (user?.firstName || user?.username || 'User') : 'Lisa'}</span>
        ...
        <div className="chat-input-area">
          ...
          {channelData && (
            <div className="csv-chip json-chip">
              ...
              <span className="csv-chip-name">Loaded channel JSON</span>
              <span className="csv-chip-meta">
                {getVideosFromChannelData(channelData).length} videos
              </span>
              <button className="csv-chip-remove" onClick={clearChannelData} aria-label="Remove JSON">×</button>
            </div>
          )}
          {channelDataError && (
            <div className="csv-chip json-chip json-chip-error">
              <span>⚠ {channelDataError}</span>
              <button className="csv-chip-remove" onClick={() => setChannelDataError(null)} aria-label="Dismiss">×</button>
            </div>
          )}
          {/* CSV chip */}
          {csvContext && (
          ...
          <div className="chat-input-row">
            ...
            {streaming ? (
              ...
              <button
                onClick={handleSend}
                disabled={!input.trim() && !images.length && !csvContext && !channelData}
              >
                Send
              </button>
            )}
